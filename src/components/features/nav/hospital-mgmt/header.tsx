...
const Header = ({ toggleSidebar }: { toggleSidebar: () => void }) => {
  const { healthcareAuth, insuranceAuth } = useStore();
  ...
  return (
    <header className="lg:min-h-[10.8rem] flex flex-col justify-center items-center bg-white border-b border-gray-200 px-4 lg:px-16 py-4 sticky top-0 z-10">
      <div className="max-w-[75.5rem] w-full mx-auto flex items-center justify-between">
        <ProfileInfo
          name={
            healthcareAuth?.details?.name ||
            insuranceAuth?.details?.insurance?.name ||
            ""
          }
          subName={"N/A"}
          profileImage={
            healthcareAuth?.details?.photo_1 ??
            healthcareAuth?.details?.photo_2 ??
            healthcareAuth?.details?.photo_3
          }
